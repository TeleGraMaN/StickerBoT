...
async def start_handler(c: Client, m: Message):
    await m.reply_text(
        "Hi, I just Generate Sticker BoT. \nMy creator @Mutyala Harshith](https://t.me/MutyalaHarshith) "
        "My Name is [GenStickerBoT](https://t.me/GenStickerBoT)  "
        "Just Send Me Text I will Send you Sticker it's not packable.",
...
async def help_handler(c: Client, m: Message):
    await m.reply_text(
        ...
        "\nMy creator [Harshith](https://t.me/MutyalaHarshith)"
        "My Source Code is [GenStickerBoT](https://github.com/TeleGraMaN/StickerBoT) "
        "add me to also in group and click /gensticker option for more info",
        disable_web_page_preview=True
    )
...
async def create_sticker(c: Client, m: Message):
    ...
    font = ImageFont.truetype("redressed.ttf", body_font_size)
    font_who = ImageFont.truetype("Buckerley.ttf", 24)
    ...
    try:
        ...
        if os.path.isfile(photo) and (photo != "harshith.jpg):
            os.remove(photo)
    except Exception as e:
        logging.error(e)


@harshith.on_message(filters.text & filters.private & (~filters.command("start") | ~filters.command("help")))
...
@harshith.on_message(filters.command(["sticker", "gensticker"]) & filters.reply & filters.group)
async def create_sticker_group_handler(c: Client, m: Message):
    s = await m.reply_text("Processing", reply_to_message_id=m.message_id)
    await create_sticker(c, m.reply_to_message)
    await s.delete()


async def main():
    await harshith.start()
    await idle()
